...
def get_tooth_region_lab(lab_img):
    ...
    Improved with more robust contour filtering and area checks.
    Returns the average LAB of the detected tooth region and its pixel area.
    """
    h_img, w_img = lab_img.shape[:2]

    # Convert LAB to BGR for better OpenCV operations (for masking)
    ...
    roi_y1, roi_y2 = h_img // 3, 2 * h_img // 3
    roi_x1, roi_x2 = w_img // 4, 3 * w_img // 4
    roi = img_bgr[roi_y1:roi_y2, roi_x1:roi_x2]

    if roi.size == 0:
        print("WARNING: ROI for enhanced segmentation is empty. Falling back to central area.")
        return get_tooth_region_lab_fallback(lab_img), 0 # Return 0 area

    hsv = cv2.cvtColor(roi, cv2.COLOR_BGR2HSV)
    
    # HSV range for white/yellowish teeth
    ...
    # Filter contours based on area and aspect ratio to remove noise/non-tooth objects
    min_area = 0.01 * roi.shape[0] * roi.shape[1] # Min 1% of ROI area
    max_area = 0.8 * roi.shape[0] * roi.shape[1]  # Max 80% of ROI area
    
    x, y, w_c, h_c = cv2.boundingRect(largest_contour)
    aspect_ratio = float(w_c) / h_c if h_c > 0 else 0
    
    # Check if the largest contour is a plausible tooth region
    if not (min_area < cv2.contourArea(largest_contour) < max_area and 0.3 < aspect_ratio < 3.0):
        print("WARNING: Largest contour does not meet size/aspect ratio criteria. Falling back to central area.")
        return get_tooth_region_lab_fallback(lab_img), 0 # Return 0 area
    
    # Create a mask for the largest contour, but apply it to the *full* image's LAB space
    final_mask_full_image = np.zeros((h_img, w_img), dtype=np.uint8)
    ...
    detected_pixel_area = np.count_nonzero(final_mask_full_image)
    ...
    return tuple(avg_lab), detected_pixel_area
...
def calculate_confidence(delta_e_value, has_reference_tab=False, image_brightness_l=50, exif_data=None, image_sharpness=0, detected_tooth_area_ratio=0):
    """
    Calculates a confidence score based on the Delta E value, presence of a reference tab,
    image brightness, simulated influence from EXIF data, image sharpness, and detected tooth area.
    """
    notes = []
    final_confidence = 0
    ...
    if isinstance(delta_e_value, (int, float)) and not np.isinf(delta_e_value) and not np.isnan(delta_e_value):
        ...
        # NEW: Sharpness influence
        # Typical sharpness values vary wildly, use a threshold relative to common ranges
        # A very blurry image might have sharpness < 100, a sharp one > 500-1000+
        sharpness_threshold_low = 150 # Below this, it's considered blurry
        sharpness_threshold_high = 600 # Above this, it's considered sharp
        
        if image_sharpness < sharpness_threshold_low:
            adjusted_confidence -= 15
            notes.append(f"Image appears blurry (Sharpness: {image_sharpness:.0f}). Accuracy may be reduced.")
        elif image_sharpness > sharpness_threshold_high:
            adjusted_confidence += 5
            notes.append(f"Image is very sharp (Sharpness: {image_sharpness:.0f}), enhancing accuracy.")
        else:
            notes.append(f"Image sharpness is adequate (Sharpness: {image_sharpness:.0f}).")

        # NEW: Detected Tooth Area Ratio influence
        # A good tooth detection should cover a reasonable portion of the image
        min_tooth_area_ratio = 0.05 # Minimum 5% of image area should be tooth
        optimal_tooth_area_ratio = 0.25 # Optimal around 25% of image area
        
        if detected_tooth_area_ratio < min_tooth_area_ratio:
            adjusted_confidence -= 20
            notes.append(f"Tooth region detected is very small ({detected_tooth_area_ratio*100:.1f}% of image). Analysis may be less reliable.")
        elif detected_tooth_area_ratio > optimal_tooth_area_ratio:
            adjusted_confidence += 5
            notes.append(f"Large, well-defined tooth region detected ({detected_tooth_area_ratio*100:.1f}% of image), boosting confidence.")
        else:
            notes.append(f"Adequate tooth region detected ({detected_tooth_area_ratio*100:.1f}% of image).")
    ...
    # Ensure there's always at least one note if no specific issues
    if not notes:
...
def detect_shades_from_image(image_path, selected_reference_tab="neutral_gray", simulated_device_profile="N/A"):
    ...
    try:
        ...
        # Extract tooth region average LAB and its area
        avg_lab_tuple_raw_tooth, detected_tooth_pixel_area = get_tooth_region_lab(lab_img)
        total_image_pixels = lab_img.shape[0] * lab_img.shape[1]
        detected_tooth_area_ratio = detected_tooth_pixel_area / total_image_pixels if total_image_pixels > 0 else 0
        ...
        # print(f"DEBUG: Detected Tooth Area Ratio: {detected_tooth_area_ratio:.2f}")
        ...
        overall_accuracy_confidence, confidence_notes = calculate_confidence(
            ...
            image_sharpness=initial_image_sharpness, # Pass initial sharpness
            detected_tooth_area_ratio=detected_tooth_area_ratio # Pass detected tooth area ratio
        )
        # print(f"DEBUG: 11. Overall Accuracy Confidence: {overall_accuracy_confidence}%")
        
        # Determine the final "Rule-based" shades
...
def save_patient_data():
    """Handles saving new patient records to the database and redirects to image upload page."""
    ...
    with next(get_session()) as session:
        ...
        try:
            ...
            session.add(patient)
            session.commit() # Commit to save the new patient
            session.refresh(patient) # Refresh to get the generated ID
...
def report_page(report_filename):
    ...
    # Extract data for rendering (convert SQLModel object to dict for easier access in template)
    patient_name = "N/A"
    with next(get_session()) as session:
        if report_data.patient_id:
            patient = session.get(Patient, report_data.patient_id)
            if patient:
...
def download_report(filename):
    ...
    user_id = g.user['id']
    with next(get_session()) as session:
        # Verify the report belongs to the logged-in user for security
        statement = select(Report).where(
...
@login_required
def submit_feedback():
    """Handles user feedback submission and logs it to the database."""
        ...
        try:
            ...
            if report_to_update and report_to_update.user_id == user_id:
                ...
                if 'feedback' not in report_to_update.detected_shades:
                    report_to_update.detected_shades['feedback'] = []
                
                report_to_update.detected_shades['feedback'].append({
                    'is_correct': is_correct,
                    'correct_shade_provided': correct_shade if not is_correct else None,
                    'timestamp': datetime.now().isoformat()
                })
                session.add(report_to_update) # Mark as changed
                ...
                return redirect(url_for('report_page', report_filename=report_to_update.report_filename))
            else:
                flash("Report not found or unauthorized to submit feedback.", 'danger')
        except Exception as e:
